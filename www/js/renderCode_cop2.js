                    ...
                    transaction.executeSql('SELECT * FROM QUESTIONS WHERE CATEGORY_ID ="' + id + '" LIMIT 0,20;', [],
                        function (transaction, results) {
                            if (results != null && results.rows != null) {
                                ...
                                for (var i = 0; i < len; i++) {
                                    ////logNow("Row = " + i + " ID = " + results.rows.item(i).id + " Data =  " + results.rows.item(i).data);
                                    html = html + "<li class='list-row "+colour[results.rows.item(i).question_no]+"' id='" + results.rows.item(i).question_no + "' onclick='renderAns(" + results.rows.item(i).question_no + ")'>";
                                    html = html + "<strong class='list-link "+colour[results.rows.item(i).question_no]+"' ontouchstart='clearSelected();'>" + results.rows.item(i).question + "</strong></li>";
                                }
                            }
                            if (ver != '' && ver < 402) {
        ...
        transaction.executeSql(qry, [],
            function (transaction, results) {
                if (results != null && results.rows != null) {
                    ...
                    for (var i = 0; i < len; i++) {
                        ////logNow("Row = " + i + " ID = " + results.rows.item(i).id + " Data =  " + results.rows.item(i).data);
                        htmlDiv = htmlDiv + "<li class='list-row  "+colour+"' id='" + results.rows.item(i).question_no + "' onclick='renderAns(" + results.rows.item(i).question_no + ")' >";
                        htmlDiv = htmlDiv + "<strong class='list-link "+colour+"' ontouchstart='clearSelected();'>" + results.rows.item(i).question + "</strong></li>";
                        if (i == (len - 1)) {
                            if (ver != '' && ver < 402) {
                    ...
                    var src = document.getElementById("qList").innerHTML;
                    var template = Handlebars.compile(src);
                    var htmlDiv = template(results);
                    if (1) {
                        //logNow("PUSHING VIEW");
                        var viewDescriptor = {
                            title: title,
...
function loadAns(id) {
    ...
    onNav();
    db.transaction(function (transaction) {
        transaction.executeSql('SELECT * FROM QUESTIONS WHERE QUESTION_NO ="' + Number(id) + '";', [],
            function (transaction, results) {
                if (results != null && results.rows != null) {
                    //logNow("Questions table: " + len + " rows found.");
                    var len = results.rows.length;
            ...
            function (transaction, results) {
                if (results != null && results.rows != null) {
                    ...
                    for (var i = 0; i < len; i++) {
                        ...
                        html = html + "<a class='list-link icon-right "+rows.item(i).colour+" white' href='#'>" + rows.item(i).folder_name.toTitleCase() + "<span id='" + i + "cts' class='list-val'>0</span></a></li>";
...
//                                                if (len > 0) {
//                                                    for (var i = 0; i < len; i++) {
//                                                        questions[i] = Number(results.rows.item(i).question_no);
//                                                    }
...
function renderAbout() {
    ...
    if (ver != '' && ver < 402) {
        ...
        htmlDiv = htmlDiv + "<p><strong>Backup:</strong></p><p>Backup your personal folders to avoid loosing them.</p>";
        htmlDiv = htmlDiv + "<p style='text-align:center;'></p><button class='button button-small button-blue block' ontouchend=\"confirmBackup();\">Backup To SD Card</button><p>&nbsp;</p>";

        htmlDiv = htmlDiv + "<p><strong>Restore:</strong></p><p>Restore your personal folders from backup. This would replace your folders with those in the backup</p>";
        htmlDiv = htmlDiv + "<p style='text-align:center;'></p><button class='button button-small button-blue block' ontouchend=\"restoreFromCardNow();\">Restore From SD Card</button><p>&nbsp;</p>";
    } else {
        ...
        htmlDiv = htmlDiv + "<p><strong>Backup:</strong></p><p>Backup your personal folders to avoid loosing them.</p>";
        htmlDiv = htmlDiv + "<p style='text-align:center;'></p><button class='button button-small button-blue block' onclick=\"confirmBackup();\">Backup To SD Card</button><p>&nbsp;</p>";

        htmlDiv = htmlDiv + "<p><strong>Restore:</strong></p><p>Restore your personal folders from backup. This would replace your folders with those in the backup</p>";
        htmlDiv = htmlDiv + "<p style='text-align:center;'></p><button class='button button-small button-blue block' onclick=\"restoreFromCardNow();\">Restore From SD Card</button><p>&nbsp;</p>";
    }
