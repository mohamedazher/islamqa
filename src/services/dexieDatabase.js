...
class DexieDatabase extends Dexie {
  constructor() {
    ...
    this.version(1).stores({
      ...
      quiz_enhancements: 'reference'  // High-quality quiz options generated by LLM, indexed by question reference
    })
    ...
    this.quiz_enhancements = this.table('quiz_enhancements')
  }
  ...
  /**
   * Get quiz enhancement (high-quality options) for a question
   * Returns null if not available (will use dynamic generation fallback)
   */
  async getQuizEnhancement(questionReference) {
    try {
      const enhancement = await this.quiz_enhancements.get(questionReference)
      return enhancement || null
    } catch (error) {
      console.error('Error getting quiz enhancement:', error)
      return null
    }
  }

  /**
   * Save quiz enhancements (high-quality options generated by LLM)
   */
  async saveQuizEnhancement(enhancement) {
    try {
      const result = await this.quiz_enhancements.put({
        reference: enhancement.reference,
        options: enhancement.options,
        explanation: enhancement.explanation,
        difficulty: enhancement.difficulty,
        tags: enhancement.tags || [],
        source: enhancement.source || 'llm-generated',
        generatedDate: Date.now(),
        ...enhancement  // Include any other fields
      })
      return result
    } catch (error) {
      console.error('Error saving quiz enhancement:', error)
      throw error
    }
  }

  /**
   * Bulk import quiz enhancements from batch
   */
  async bulkImportEnhancements(enhancements) {
    try {
      ...
      console.log(`✅ Imported ${enhancements.length} quiz enhancements`)
      return enhancements.length
    } catch (error) {
      console.error('Error bulk importing enhancements:', error)
      throw error
    }
  }
  ...
  async getEnhancementStats() {
    try {
      const count = await this.quiz_enhancements.count()
      ...
      return { enhanced: 0, total: 0, percentage: 0 }
    }
  }
  ...
  async clearAllData() {
    try {
      await this.transaction('rw', [
        ...
        await this.quiz_enhancements.clear()
      })
