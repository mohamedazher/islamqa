...
/**
 * Consolidate all batch-002 sub-batch outputs into batch-002-output.json
 */
...
const OUTPUT_FILE = path.join(BATCHES_DIR, 'batch-002-output.json')

console.log('🔨 Consolidating batch-002 sub-batch outputs...\n')
...
for (let i = 1; i <= 20; i++) {
  ...
  const filePath = path.join(BATCHES_DIR, `batch-002-sub-${subBatchId}-output.json`)

  if (!fs.existsSync(filePath)) {
    console.log(`❌ Missing: batch-002-sub-${subBatchId}-output.json`)
    continue
  }
...
console.log(`📁 Output: quiz-generation/batches/batch-002-output.json`)
