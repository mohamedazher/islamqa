...
/**
 * Split batch-003-input.json into 20 sub-batches of 10 questions each
 */

const fs = require('fs')
const path = require('path')

const BATCH_FILE = path.join(__dirname, 'batches/batch-003-input.json')
...
for (let i = 0; i < numSubBatches; i++) {
  ...
  const subBatchPath = path.join(BATCHES_DIR, `batch-003-sub-${subBatchId}-input.json`)
  fs.writeFileSync(subBatchPath, JSON.stringify(subBatch, null, 2))

  console.log(`✅ Created batch-003-sub-${subBatchId}-input.json (${subQuestions.length} questions)`)
}
