...
/**
 * Split batch-001-input.json into 10 sub-batches of 10 questions each
...
const BATCH_FILE = path.join(__dirname, 'batches/batch-001-input.json')
...
for (let i = 0; i < numSubBatches; i++) {
  ...
  const subBatchPath = path.join(BATCHES_DIR, `batch-001-sub-${subBatchId}-input.json`)
  fs.writeFileSync(subBatchPath, JSON.stringify(subBatch, null, 2))

  console.log(`✅ Created batch-001-sub-${subBatchId}-input.json (${subQuestions.length} questions)`)
}

console.log(`\n✅ Split complete!`)
console.log(`📁 Files: quiz-generation/batches/batch-001-sub-01 through batch-001-sub-10`)
