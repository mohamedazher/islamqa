...
function generateTags(title, question, answer) {
  ...
  // Core topic detection with priority scoring
  const tagPatterns = [
  ...
  // Ensure we have fiqh tag for ruling-related questions
...
function determineRuling(title, question, answer) {
  ...
  // Strong indicators in order of precedence
...
// Generate comprehensive summary (60-100 words)
function generateSummary(title, question, answer) {
  ...
  // Determine question type and create appropriate intro
  if (/^is it (permissible|halal|haram|allowed|forbidden)/i.test(title)) {
    ...
    summary = `This explains ${cleanTitle.toLowerCase()}. `;
  } else if (/^what is/i.test(title)) {
    summary = `This explains ${cleanTitle.toLowerCase().replace(/^what is /i, '')} in Islamic teaching. `;
  } else if (/^does/i.test(title)) {
    summary = `This addresses whether ${cleanTitle.toLowerCase().replace(/^does /i, '')}. `;
  } else {
    summary = `This addresses ${cleanTitle.toLowerCase()}. `;
  }

  // Add key points from answer
  if (sentences.length > 0) {
    // Find the most informative sentence (usually one with ruling keywords)
    let bestSentence = sentences[0];
    for (const s of sentences.slice(0, 5)) {
      if (/(permissible|obligatory|forbidden|recommended|scholars|ruling|evidence|proof)/i.test(s)) {
        bestSentence = s;
        break;
      }
    }
    ...
    let point = bestSentence.charAt(0).toUpperCase() + bestSentence.slice(1);
    if (!point.endsWith('.')) point += '.';
    summary += point;
  }

  // Add ruling context based on detected ruling
  const ruling = determineRuling(title, question, answer);
  if (ruling === 'haram') {
    summary += ' The scholars indicate this is prohibited in Islam.';
  } else if (ruling === 'wajib') {
    summary += ' This is considered obligatory upon Muslims.';
  } else if (ruling === 'mustahabb') {
    summary += ' This is recommended but not obligatory.';
  } else if (ruling === 'makruh') {
    summary += ' This is discouraged though not sinful.';
  } else if (ruling === 'halal') {
    summary += ' This is permissible in Islam.';
  }

  // Add second key point if summary is too short
  const words = summary.split(/\s+/);
  if (words.length < 55 && sentences.length > 1) {
    let additionalPoint = sentences[1].trim();
    if (additionalPoint.length > 20 && additionalPoint.length < 200) {
  ...
  if (finalWords.length > 100) {
    summary = finalWords.slice(0, 97).join(' ') + '...';
  }
...
function processQuestions() {
  ...
  // Print sample
  const sampleRefs = Object.keys(summaries).slice(0, 3);
  console.log('\nSample output:');
